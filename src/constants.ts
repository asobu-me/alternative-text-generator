...
/**
 * The Gemini model, for every request. Deliberately not configurable: the prompts and
 * the length constraints are tuned against this one model, and a per-user model would
 * make every generated result depend on a setting nobody can see from the output.
 *
 * Changing it here is only half the change — `ALLOWED_MODELS` in `proxy/worker.js` must
 * match AND the proxy must be redeployed, or the default (no user key) path answers
 * 400 "Unsupported model" for everyone.
 */
...
export const PROXY_CONFIG = {
     ...
     *
     * A self-owned host under `api.asobu.me`, routed to the `auto-alt-writer-proxy`
     * Worker. The `/auto-alt-writer/` path segment namespaces this app, leaving room to
     * route other apps under the same host later. Baked into every shipped build, so it
     * is effectively permanent — changing it strands already-installed extensions.
     */
    DEFAULT_ENDPOINT: 'https://api.asobu.me/auto-alt-writer/',
    /**
     ...
     */
    CLIENT_TOKEN: 'auto-alt-writer-public-client-v1',
} as const;
