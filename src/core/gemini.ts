...
async function fetchDirect(
    ...
    if (!GEMINI_DIRECT.MODEL_NAME_PATTERN.test(safeModel)) {
        throw new InvalidRequestError(
            vscode.l10n.t('"{0}" is not an allowed model name. This is a bug in the extension — please report it.', safeModel),
            400
        );
    }
...
/**
 * Run an async generation call with automatic retry for retryable errors.
 * Only retries network errors and server errors (5xx); rate-limit errors (429)
 * and cancellations are thrown immediately.
 */
async function withRetry<T>(
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
        try {
            ...
            console.log(`[Auto ALT Text Writer] Retrying after network/server error (attempt ${attempt + 1}/${maxRetries}, waiting ${waitTime}ms)`);
...
export async function generateAltTextWithRetry(
    ...
    return withRetry(
        () => generateAltText(base64Image, mimeType, mode, model, token, surroundingText),
        token,
        maxRetries
    );
}
...
export async function generateVideoAriaLabelWithRetry(
    ...
    return withRetry(
        () => generateVideoAriaLabel(base64Video, mimeType, model, token, surroundingText, mode),
        token,
        maxRetries
    );
}
