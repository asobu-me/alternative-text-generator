...
export function detectAllTags(
    ...
    if (selectedText.length > maxSearchLength) {
        vscode.window.showWarningMessage(vscode.l10n.t('The selection is too large to scan for tags. Select a smaller range and try again.'));
        return tags;
    }
    ...
    while ((match = imgRegex.exec(selectedText)) !== null) {
        if (Date.now() - startTime > TAG_DETECTION.SEARCH_TIMEOUT_MS) {
            vscode.window.showWarningMessage(vscode.l10n.t('Tag detection timed out. Select a smaller range and try again.'));
            break;
        }
